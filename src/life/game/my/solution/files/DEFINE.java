package life.game.my.solution.files;

public class DEFINE
{
    public static final String SYMBOL_GROUND = "g";
    public static final String SYMBOL_WILK = "W";
    public static final String SYMBOL_OWCA = "O";
    public static final String SYMBOL_LIS = "L";
    public static final String SYMBOL_ZOLW = "Z";
    public static final String SYMBOL_ANTYLOPA = "A";
    public static final String SYMBOL_CZLOWIEK = "C";
    public static final String SYMBOL_TRAWA = "T";
    public static final String SYMBOL_MLECZ = "M";
    public static final String SYMBOL_GUARANA = "G";
    public static final String SYMBOL_WIKLCZEJAGODY = "w";
    public static final String SYMBOL_BARSZCZSOSNOWSKIEGO = "B";
    public static final int WILK_AMOUNT = 5;
    public static final int CZLOWIEK_AMOUNT = 1;
    public static final int OWCA_AMOUNT = 5;
    public static final int LIS_AMOUNT = 4;
    public static final int ZOLW_AMOUNT = 4;
    public static final int ANTYLOPA_AMOUNT = 4;
    public static final int TRAWA_AMOUNT = 3;
    public static final int MLECZ_AMOUNT = 3;
    public static final int GUARANA_AMOUNT = 3;
    public static final int WILCZEJAGODY_AMOUNT = 3;
    public static final int BARSZCZSOSNOWSKIEGO_AMOUNT = 3;

    public DEFINE(){
    }
}
